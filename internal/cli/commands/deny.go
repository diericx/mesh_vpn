...
// Deny denies traffic to/from a peer
func Deny(args []string) error {
	if len(args) < 2 {
		return fmt.Errorf("usage: mvpn deny <incoming|outgoing> <peer-name>")
	}
	...
	if direction == "incoming" {
		aclRule.Incoming = false
	} else {
		aclRule.Outgoing = false
	}
	...
	if direction == "incoming" {
		if err := fwMgr.BlockIncoming(peer.WireGuardIP); err != nil {
			return fmt.Errorf("failed to apply firewall rule: %w", err)
		}
	} else {
		if err := fwMgr.BlockOutgoing(peer.WireGuardIP); err != nil {
			return fmt.Errorf("failed to apply firewall rule: %w", err)
		}
	}

	fmt.Printf("Denied %s traffic for peer %s\n", direction, peerName)
